...
buildList start count end = buildListHelper start count [end]
  where buildListHelper start 0 result = result
        buildListHelper start count result = buildListHelper start (count-1) (start:result)
...
sums :: Int -> [Int]
sums 0 = []
sums i = mymap sumsHelper [1..i]
  where mymap _ [] = []
        mymap f (x:xs) = f x : mymap f xs
        sumsHelper 1 = 1
        sumsHelper i = i + sumsHelper (i - 1)
...
sumsOf [] = []
sumsOf [a] = [a]
sumsOf (x:xs) = x:mymap (+x) (sumsOf xs)
  where mymap _ [] = []
        mymap f (x:xs) = f x : mymap f xs
